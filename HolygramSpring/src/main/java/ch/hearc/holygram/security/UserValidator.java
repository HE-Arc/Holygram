...
public class UserValidator implements Validator {
    ...
    public void validate(Object o, Errors errors) {
        ...
        if (userForm.getPassword().length() < 8 || userForm.getPassword().length() > 32) {
            errors.rejectValue("password", "Size.userForm.password", "The password must be between 8 and 32 characters");
        }

        if (!userForm.getPasswordConfirm().equals(userForm.getPassword())) {
            errors.rejectValue("passwordConfirm", "Diff.userForm.passwordConfirm", "The confirmation password isn't the same as password");
        }
    }
}